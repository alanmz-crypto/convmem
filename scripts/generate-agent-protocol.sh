...
# --- Crush rules (Tier A — shell + MCP; soak #8: MCP-only ignored) ---
{
  ...
  extract_section MCP_AFTER_TIER_A
  echo ""
  echo "**Qwen3.7-Max (default) / DeepSeek V4 in Crush:** do **not** skip this ritual on alien \"project state\" queries — never start with \`ls\`, git, or docker until convmem steps above complete."
  echo ""
  echo "**MCP hang:** if \`mcp_convmem_*\` stalls or returns \`context canceled\`, cancel and use shell \`convmem \"…\"\` / \`convmem ask \"…\"\` — do not retry the same hung MCP call in a loop."
  echo ""
  echo "## Session close"
  echo ""
  extract_section SESSION_CLOSE
  ...
  echo ""
  echo "- You are **Crush lane**; never call yourself by the provider/model name in handoff text (Qwen/DeepSeek/Kimi are runtime weights only)."
  echo "- Default large model: **Qwen3.7-Max** (Alibaba Singapore). Coding specialist: **Kimi K2.7 Code**."
  ...
  extract_section BOUNDED_AUTONOMY
  echo ""
  echo "## Standing autonomous actions (Kiro-approved 2026-08-14)"
  echo ""
  echo "Crush may perform the following **without Ryan prompting** — no ask, no wait. All five envelope conditions must hold:"
  echo ""
  echo "1. **Reversible** — a single \`git revert\` or branch reset undoes it."
  echo "2. **Non-\`main\`** — feature branches only; never merge, never force-push."
  echo "3. **No new authorization** — the surfaced/maintained work is already authorized elsewhere."
  echo "4. **No external effect** — no API calls, service restarts, config changes, or deployments."
  echo "5. **Observable** — shows up in \`git log\` on the branch."
  echo ""
  echo "**Named actions (approved):**"
  echo ""
  echo "| Action | Trigger | Scope |"
  echo "|--------|---------|-------|"
  echo "| Update \`LATEST.md\` | Watchdog/brief flags STALE HANDOFF or Active P0 | Add bullet, link authorized handoff, roll date — content-only pointer work |"
  echo "| Fix false-positive alarms | A known-scaffolding file trips a heuristic (e.g. \`HANDOFF-TEMPLATE.md\` in staleness check) | One-line skip-set addition + test; literal filename only, no globs |"
  echo "| Typo/formatting fix in own output | Noticed in same session, same branch | Doc Crush authored in same branch; never others' docs |"
  echo ""
  echo "**Not covered (still needs Ryan or Kiro):**"
  echo ""
  echo "- Authoring new handoff content for work not yet authorized"
  echo "- Any change to runtime behavior (code logic, thresholds, config)"
  echo "- Anything on \`main\`"
  echo "- New external commitments (PR creation, ledger record, deployment)"
  echo "- Touching docs authored by another lane on a different branch"
  echo ""
  echo "**Extending this list:** Propose a new named action to Kiro. One-line approval adds it here. Do not extrapolate from existing entries."
  echo ""
  echo "## Response TL;DR"
  echo ""
  extract_section RESPONSE_TLDR

...
from incremental_jsonl import DURABLE_TRANSITIONS, IncrementalJsonlCoordinator, IncrementalJsonlError
...
def test_c4_isolation_dedupe_outside_role_refused_before_chmod(
    tmp_path: Path, monkeypatch
) -> None:
    boundary, env = isolated_env(tmp_path)
    apply_env(monkeypatch, env)
    ...
    coordinator.run()
    outside = boundary.layout["sources"] / "not-dedupe"
    outside.mkdir(parents=True, exist_ok=True)
    os.chmod(outside, 0o755)
    before_mode = stat.S_IMODE(outside.stat().st_mode)
    (outside / "chroma").mkdir()
    coordinator.cfg["index"]["chroma_dir"] = str(outside / "chroma")
    with pytest.raises(IncrementalJsonlError, match="outside granted role"):
    ...
    coordinator.cfg["index"]["chroma_dir"] = str(boundary.layout["chroma"])
    granted = Path(boundary.layout["dedupe"]) / "dedupe_queue.jsonl"

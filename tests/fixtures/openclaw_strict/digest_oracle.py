"""Digest/ID oracle A — normative LP/H with NFC + field grammar (T0b).

Fixture-only. Does not import production modules. Pinned vectors compare to
these recomputations inside the exact runner.

LP(value) = uint32_be(len(UTF8(NFC(value)))) || UTF8(NFC(value))
H(tag, fields...) = SHA256(ASCII(tag) || 0x00 || LP(field1) || ...)

Prefixes find2_/choice2_/check2_ and obs2_/dec2_/ver2_ are derived from the
validated subject/record kind — callers cannot mint kind/prefix mismatches.
Strict string fields require already-canonical NFC raw input (reject otherwise);
...
class DigestOracleError(ValueError):
    """Oracle A rejection."""
...
def _reject_surrogates(value: str) -> None:
    for ch in value:
        o = ord(ch)
        if 0xD800 <= o <= 0xDFFF:
            raise DigestOracleError("surrogate")


def _require_already_nfc(value: str, *, field: str) -> None:
    if not isinstance(value, str):
        raise DigestOracleError(f"not_string:{field}")
    _reject_surrogates(value)
    if value != unicodedata.normalize("NFC", value):
        raise DigestOracleError(f"non_nfc:{field}")


def _reject_float(value: Any, *, field: str) -> None:
    if isinstance(value, float):
        raise DigestOracleError(f"float_forbidden:{field}")


def lp(value: str) -> bytes:
    """Normative LP: length-prefix UTF-8 of NFC(value). Does not gate grammar."""
    if not isinstance(value, str):
        raise DigestOracleError("lp_not_string")
    _reject_surrogates(value)
    raw = unicodedata.normalize("NFC", value).encode("utf-8")
    if len(raw) > 0xFFFFFFFF:
        raise DigestOracleError("lp_too_long")
...
def H(tag: str, fields: Sequence[str]) -> str:  # pylint: disable=C0103  # protocol hash primitive name; oracle parity
    ...
    invent kind/prefix combinations.
    """
    if not isinstance(tag, str):
        raise DigestOracleError("tag_not_string")
    try:
        tag_bytes = tag.encode("ascii")
    except UnicodeEncodeError as exc:
        raise DigestOracleError("tag_not_ascii") from exc
    if tag_bytes.decode("ascii") != tag:
        raise DigestOracleError("tag_not_ascii")
    parts = [tag_bytes, b"\x00"]
    for field in fields:
        parts.append(lp(field))
...
def _validate_site_normalized(site: str) -> str:
    """Require already-normalized ASCII hostname; underscore/path rejected."""
    _require_already_nfc(site, field="authority_site")
    if not site or site != site.lower():
        raise DigestOracleError("authority_site_not_normalized")
    if "_" in site or "/" in site or ":" in site or "@" in site or " " in site:
        raise DigestOracleError("authority_site_grammar")
...
def owner_digest(*, scope_sha256: str, registry_sha256: str, project_binding_id: str) -> str:
    ...
    if not SHA256_LABELED_RE.fullmatch(scope_sha256):
        raise DigestOracleError("scope_sha256_grammar")
    if not SHA256_LABELED_RE.fullmatch(registry_sha256):
        raise DigestOracleError("registry_sha256_grammar")
    obj = {
...
def logical_id(
    *,
    ...
    """Mint find2_/choice2_/check2_ from validated subject_kind only."""
    _require_already_nfc(project_binding_id, field="project_binding_id")
    _require_already_nfc(source_identity, field="source_identity")
    ...
    if subject_kind == "verification":
        if not target_assertion_id_or_empty:
            raise DigestOracleError("verification_requires_target")
        ...
        if not re.fullmatch(
            ...
            raise DigestOracleError("target_assertion_grammar")
    else:
        if target_assertion_id_or_empty != "":
            raise DigestOracleError("non_verification_target_must_be_empty")
    digest = _H_fixed(
        TAG_LOGICAL,
        (
            project_binding_id,
...
def assertion_id(
    *,
    ...
    """Mint obs2_/dec2_/ver2_ from validated record_kind only."""
    _require_already_nfc(project_binding_id, field="project_binding_id")
    _require_already_nfc(source_registration_id, field="source_registration_id")
    ...
    if record_kind not in RECORD_PREFIX:
        raise DigestOracleError(f"record_kind:{record_kind}")
    ...
    if not logical_id_value.startswith(expected_logical):
        raise DigestOracleError("kind_prefix_mismatch")
    if not re.fullmatch(r"evt_[0-9a-f]{64}", source_event_id):
        raise DigestOracleError("source_event_id_grammar")
    digest = _H_fixed(
        TAG_ASSERTION,
        (
            project_binding_id,
...
def citation_ref(
    *,
    ...
    if not SHA256_LABELED_RE.fullmatch(provenance_commitment):
        raise DigestOracleError("provenance_commitment_grammar")
    obj = {
...
def receipt_ref(receipt_payload_sha256_hex64: str) -> str:
    """receipt_ref = capture_ + payload hash hex (no sha256: prefix in address)."""
    ...
    if not HEX64_RE.fullmatch(receipt_payload_sha256_hex64):
        raise DigestOracleError("receipt_payload_hex_grammar")
    return "capture_" + receipt_payload_sha256_hex64


def public_handle(*, public_ref: str, stored_external_id: str) -> str:
    """Concatenation only — not a digest algorithm.

    cm1.<32hex public_ref>.<validated stored external ID>
    """
    ...
    if not PUBLIC_REF_RE.fullmatch(public_ref):
        raise DigestOracleError("public_ref_grammar")
    if not STORED_ID_RE.fullmatch(stored_external_id):
        raise DigestOracleError("stored_external_id_grammar")
    handle = f"cm1.{public_ref}.{stored_external_id}"
    if len(handle) > 200:
        raise DigestOracleError("handle_too_long")
    if not HANDLE_RE.fullmatch(handle):
        raise DigestOracleError("handle_grammar")
    return handle
...
def exclude_named_field_bytes(obj: dict[str, Any], field: str) -> bytes:
    ...
    if not isinstance(obj, dict):
        raise DigestOracleError("exclude_not_object")
    if not isinstance(field, str) or not field:
        raise DigestOracleError("exclude_field_invalid")

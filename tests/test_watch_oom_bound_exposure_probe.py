...
from brief import EXPOSURE_WINDOW_METADATA_KEYS
from doctor import (
...
from tests.watch_oom_exposure_hermetic import (
    ...
    exposure_cfg,
    trap_iter_collection_metadata_rows,
...
class ExposureFailSoftTests(unittest.TestCase):
    ...
    def test_brief_chain_publishes_advisory_due_row(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            reg = write_exposure_register(tmp)
            out = tmp / "brief.md"
            with freeze_brief_probes(pin_standing_register=False), patch(
                "doctor._standing_register_path", return_value=reg
            ), patch(
                "doctor._exposure_window_probe",

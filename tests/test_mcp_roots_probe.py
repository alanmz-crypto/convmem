...
class McpRootsHelpers(unittest.TestCase):
    ...
    def test_normalize_bare_path_to_file_uri(self):
        uri = mcp_server._normalize_root_uri(str(REPO))
        self.assertTrue(uri.startswith("file://"))
        self.assertIn(str(REPO), uri)
    ...
    def test_root_uris_reject_home_and_tmp(self):
        self.assertFalse(
            mcp_server._root_uris_indicate_project([f"file://{Path.home()}"])
        )
        self.assertFalse(

...
| Pre-live-write snapshot gate (fail-closed) | (a) — **mechanized + boundary locked (2026-07-07), complete-data coverage (2026-07-23)** | `restic_gate.ensure_chroma_snapshot_for_live_write()` is called inline before the two durable/overwrite Chroma paths — `add --upsert` (`convmem.py:355–358`) and `record --approve-last` (`convmem.py:919–924`) — and fail-closes on any Restic error (`restic_gate.py:13–33`). **Trigger boundary (Option 1, adopted; Option 2 "gate every mutation" declined):** `convmem index` and plain `add` (no `--upsert`) are append-only + reindexable, so intentionally ungated; `CONVMEM_SKIP_RESTIC_GATE=1` is the deliberate test/lab hatch. The Restic target is the complete ConvMem data root (canonical ledgers and sidecars plus the Chroma projection), with only Git worktrees and disposable restore runs excluded. Behavioral regression test `tests/test_write_gate_effect.py` forces the gate to fail and asserts each path aborts and leaves the corpus unchanged (effect, not reference — closes the position-vs-presence wiring hole). Boundary documented in ROADMAP + RECOVER and logged to the ledger. |

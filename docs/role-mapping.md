...
| Negative controls as standard practice | (c) — **live trigger (2026-07-29)** | `eval-negative-control-coverage` checks every non-exempt LLM-judge eval calls the shared runtime known-bad control; helper effect tests and eval exit behavior prove it gates unusable judge evidence. |
...
- **Layer 2 current state:** 13 open rows carry live triggers, including both
  eval wiring probes (`eval-provenance-wiring` and
  `eval-negative-control-coverage`); 2 charter-standing rows remain
  manual-by-design; and 3 rows are closed after mechanization. **Register end
  state: 13 open / 2 standing / 3 closed (18 total).** The JSON register is the
  source of truth; this summary no longer enumerates every row and therefore
  cannot silently preserve a stale trigger list.

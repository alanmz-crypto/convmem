...
- **CG-2 authority migration — MERGED; legacy-only gateway soak GRANT RECORDED; soak completion UNRESOLVED (2026-08-15, [#186](https://github.com/alanmz-crypto/convmem/pull/186)):** Who/What: T1–T5 serving authority, source reconciliation, logical accounting, mixed-mode proof, rehearsal + VERIFY/RUNBOOK on `main`. When: squash-merged `2f427fc` 2026-08-15. Why: prove CG-2 gateway on live corpus before any generational owner grant. How: `owners=0` on live doctor; startup reconciliation + `eval-retrieval.py` PASS at soak start; pause on [`RUNBOOK-cg2-production-activation.md`](../plans/RUNBOOK-cg2-production-activation.md). Grant evidence: LATEST records Ryan GATE granted the legacy-only soak (all owners LEGACY; no fence/pointer/GC); **no repository evidence proves the ~48h soak completed, continuing execution, or V8b closure.** **Resume state:** grant recorded; **soak completion UNRESOLVED** — **not** authorized for first owner, activation manifest, GC, or shadow enable. Do not close V8b or claim soak success until completion/verification is evidenced. Desktop checklist: `~/Desktop/CG2-after-soak-checklist.md` (not in repo). **Do not** use this chat cache — read VERIFY/RUNBOOK + doctor at resume.

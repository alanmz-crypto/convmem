...
- **Claude Watch Parity — Gate 2 plan KIRO PASS / EXECUTE RYAN-GATED:** Kiro PASSed Codex's [Gate 2 plan](CODEX-2026-09-18-claude-watch-parity-gate2-execute.md) and [arc brief](../plans/STATUS-claude-watch-parity.md) at exact reviewed commit `a811f58` with four Execute conditions: assert the shared `CONVMEM_INCREMENTAL_ROOT` isolation set and unchanged Kiro/Codex behavior; recheck the merged Gate 1 mapper before editing; keep the live canary `NOT_RUN` absent a Ryan grant and inactive watcher; lock the production-code diff to the Claude adapter and format registry. **Next:** Ryan decides the bounded isolated Execute grant; Cursor starts only after Gate 1 lands and the grant exists. No live-source canary, `KIRO_ROUTE_FORMATS` change, or `[sources].paths` / `[watch].extra_paths` change is authorized. The reviewed plan text remains unchanged on `plan/2026-09-18-claude-watch-parity`.

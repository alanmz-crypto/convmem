...
**Updated:** 2026-08-21 (Arc Trapdoor Hunt closed-T3 main integration candidate; Runway Ledger CLOSED)

- **Arc Trapdoor Hunt — CLOSED T3 main-integration candidate (draft [PR #221](https://github.com/alanmz-crypto/convmem/pull/221), 2026-08-21):** Who/What: Codex mechanically composed frozen closed-T3 tip `3ac2384d484bdb99d4ecd23f26c17264a7246adb` with a lane created from main `8df2849959d01f9c41542a6388388e049f43b74c` and refreshed through `94169be5087a9064678fd0f6c52b77a7fd7fe4ae`. When: deterministic validation passed on the pre-refresh implementation-equivalent candidate; exact-SHA review remains gated. Why: T3 must reach main without rewriting its frozen history or changing Runway Ledger semantics. How: regenerated writer inventories, preserved current CI/protocol surfaces, and retained all deferred gates as unauthorized. **Resume state:** complete exact-SHA Kiro/Copilot review, then stop for Ryan before merge. [`STATUS-dependability-provenance.md`](../plans/STATUS-dependability-provenance.md)
...
- **CG-2 authority migration — MERGED; legacy-only gateway soak GRANT RECORDED; soak completion UNRESOLVED (2026-08-15, [#186](https://github.com/alanmz-crypto/convmem/pull/186)):** Who/What: T1–T5 serving authority, source reconciliation, logical accounting, mixed-mode proof, rehearsal + VERIFY/RUNBOOK on `main`. When: squash-merged `2f427fc` 2026-08-15. Why: prove CG-2 gateway on live corpus before any generational owner grant. How: `owners=0` on live doctor; startup reconciliation + `eval-retrieval.py` PASS at soak start; pause on [`RUNBOOK-cg2-production-activation.md`](../plans/RUNBOOK-cg2-production-activation.md). Grant evidence: LATEST records Ryan GATE granted the legacy-only soak (all owners LEGACY; no fence/pointer/GC); that grant satisfies V8b (PASS). **No repository evidence proves the ~48h soak completed successfully.** **Resume state:** soak completion UNRESOLVED — **not** authorized for first owner, activation manifest, GC, or shadow enable. Do not claim soak success until completion/verification is evidenced. Desktop checklist: `~/Desktop/CG2-after-soak-checklist.md` (not in repo). **Do not** use this chat cache — read VERIFY/RUNBOOK + doctor at resume.

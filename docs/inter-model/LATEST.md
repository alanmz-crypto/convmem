...
- **CodeQL Complex Therapy — SHA-BOUND HARDENING HANDOFF READY FOR KIRO/Ryan REVIEW (2026-08-16, `plan/2026-08-16-codeql-complex-therapy`):** Who/What: Codex added producer-identity rejection, an explicit GitHub server-side mediation boundary, recurring-attestation requirements, a path-scoped latency alternative, and a mandatory independent SHA-lineage review after a transient handoff typo. When: review package is full SHA `c74c7f8611ac0bf563618270c2c3244715df7d67`; planning evidence remains Protect Main `19156572` and PR #197 head `740424884f8921f1586f6b82648a0a290be40836`; no implementation or disposable PR exists. Why: the plan must test integration-id binding, distinguish one-time evidence from continuous enforcement, and require Git-based identity verification rather than trusting pasted SHA strings. How: Kiro reviews the named package, independently resolves the fetched remote carrier, and records the `9dfaa...` typo / `790d...` correction; Ryan has accepted the all-three context set, inherited GHAS `CodeQL` semantics, blanket latency, the exact disposable fixture authorization, conditional producer-probe authorization, and recurring-attestation ownership/cadence (`OWNER=Ryan`; `CADENCE=quarterly + configuration-drift trigger`; Cursor evidence; Kiro exception review). [`CODEX-2026-08-16-codeql-complex-therapy-planning-handoff.md`](CODEX-2026-08-16-codeql-complex-therapy-planning-handoff.md) · [`ARCHITECTURE-codeql-complex-therapy.md`](../plans/ARCHITECTURE-codeql-complex-therapy.md) · [`EXECUTION-codeql-complex-therapy.md`](../plans/EXECUTION-codeql-complex-therapy.md) · [`VERIFY-codeql-complex-therapy.md`](../plans/VERIFY-codeql-complex-therapy.md) · [`STATUS-codeql-complex-therapy.md`](../plans/STATUS-codeql-complex-therapy.md). **Resume state:** `CONTEXT + GHAS + BLANKET LATENCY + DISPOSABLE FIXTURE + CONDITIONAL PRODUCER PROBE + ATTESTATION OWNER/CADENCE ACCEPTED` — Execute authorization remains open.
...
- **CG-2 authority migration — MERGED; legacy-only gateway soak IN_PROGRESS (2026-08-15, [#186](https://github.com/alanmz-crypto/convmem/pull/186)):** Who/What: T1–T5 serving authority, source reconciliation, logical accounting, mixed-mode proof, rehearsal + VERIFY/RUNBOOK on `main`. When: squash-merged `2f427fc` 2026-08-15; Ryan GATE granted legacy-only soak same day (all owners LEGACY; no fence/pointer/GC). Soak target: watch stable ~48h from 2026-08-15. Why: prove CG-2 gateway on live corpus before any generational owner grant. How: `owners=0` on live doctor; startup reconciliation + `eval-retrieval.py` PASS at soak start; pause on [`RUNBOOK-cg2-production-activation.md`](../plans/RUNBOOK-cg2-production-activation.md). **Resume state:** `IN_PROGRESS` soak — **not** authorized for first owner, activation manifest, GC, or shadow enable. Post-soak agent work: close V8b in [`VERIFY-cg2-production-activation.md`](../plans/VERIFY-cg2-production-activation.md), update this bullet, then wait for Ryan first-owner grant. Desktop checklist: `~/Desktop/CG2-after-soak-checklist.md` (not in repo). **Do not** use this chat cache — read VERIFY/RUNBOOK + doctor at resume.

- **CI Behavioral Merge Gate — MERGED; enforcement NEGATIVE CONTROL PASS (2026-08-16, [#187](https://github.com/alanmz-crypto/convmem/pull/187) + disposable [#188](https://github.com/alanmz-crypto/convmem/pull/188)):** Who/What: hermetic pytest job in GitHub Actions; `Protect Main` requires `pytest (3.12)` alongside `pylint (3.12)`. When: #187 merged as `c2c6429` on 2026-08-15; #188 tested and closed on 2026-08-16. Why: prove a known-bad candidate cannot enter `main` through the ordinary path. How: #188 head `02ee739` produced a red required `pytest (3.12)` check and `mergeStateStatus=BLOCKED`; admin bypass was observed as available but not exercised. **Arc CI Kryptonite technical evidence is complete; Ryan docs-merge GATE remains.** See [`VERIFY-ci-behavioral-merge-gate.md`](../plans/VERIFY-ci-behavioral-merge-gate.md) and [`CODEX-2026-08-15-ci-behavioral-merge-gate-closeout-handoff.md`](CODEX-2026-08-15-ci-behavioral-merge-gate-closeout-handoff.md).

...
- **Claude Watch Parity — NEXT: CURSOR HERMETIC GATE 1 SMOKE PREP:** Start with the [real-smoke handoff](CODEX-2026-09-18-claude-gate1-real-smoke-handoff.md). [PR #310](https://github.com/alanmz-crypto/convmem/pull/310) merged the Kiro-reviewed on-demand Claude adapter at `aadf137`, but its original real-transcript `files_processed=1` / nonzero-units acceptance remains `NOT_RUN`. Cursor proves a scratch-isolated CLI path with synthetic data and stops before live access. Ryan must name one source and separately grant read-only fingerprinting and a bounded real run. The [Gate 2 plan](CODEX-2026-09-18-claude-watch-parity-gate2-execute.md) received Kiro PASS at `a811f58`; its isolated Execute, live canary, and production watch promotion remain separate Ryan gates. See the [arc brief](../plans/STATUS-claude-watch-parity.md).

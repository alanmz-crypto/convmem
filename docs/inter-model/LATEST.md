...
- **CodeQL Complex Therapy — planning package READY FOR RYAN/KIRO REVIEW (2026-08-16, `plan/2026-08-16-codeql-complex-therapy`):** Who/What: Codex authored the architecture, execution, VERIFY, and STATUS package to make CodeQL a required merge gate on `main` (successor to Pinwheel). When: live capture used Protect Main `19156572` and PR #191 head `35e090d`; no implementation or disposable PR exists. Why: CodeQL passes on PRs but Protect Main only requires pylint + pytest. How: proposed ruleset-only Execute adds exact contexts `Analyze (actions)`, `Analyze (python)`, and `CodeQL`, preserves strict policy, then Cursor proves normal green and Ryan-authorized disposable red/missing behavior. Ryan reviews; separate Execute and disposable-control grants follow. [`ARCHITECTURE-codeql-complex-therapy.md`](../plans/ARCHITECTURE-codeql-complex-therapy.md) · [`EXECUTION-codeql-complex-therapy.md`](../plans/EXECUTION-codeql-complex-therapy.md) · [`VERIFY-codeql-complex-therapy.md`](../plans/VERIFY-codeql-complex-therapy.md) · [`STATUS-codeql-complex-therapy.md`](../plans/STATUS-codeql-complex-therapy.md). **Resume state:** `PLANNING READY` — Execute not authorized.

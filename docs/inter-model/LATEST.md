...
**Updated:** 2026-07-19 (Stage 4 Task 1 shipped — PR #46 merged)
**Live counts:** run `convmem brief` — do not trust stale numbers here.

## Active handoff

- **Stage 4 Task 1 shipped (2026-07-19):** Crush builder digests demoted to on-demand — PR [#46](https://github.com/alanmz-crypto/convmem/pull/46) merged to main (`bd037b8`). Builder-reference pointer (`rules/builder-reference-pointer.md`) replaces full digest copies in Crush `global_context_paths`. Agents load digests on-demand per scope rules instead of at session start. Architecture: [`ARCHITECTURE-stage4-context-compression.md`](../plans/ARCHITECTURE-stage4-context-compression.md) — execution: [`EXECUTION-stage4-context-compression.md`](../plans/EXECUTION-stage4-context-compression.md).
- **Gate 1 embedding-eval harness (2026-07-19) — Codex PASS, merge pending:** Tip `dbac4327a52b13e918a72e0a633deb496fb11834` on `feat/2026-07-19-embed-eval-harness`. PR [#44](https://github.com/alanmz-crypto/convmem/pull/44) mergeable/CLEAN (pylint + Approval Agent green; Bugbot skipped/neutral). Codex independent diff + full suite PASS (626 passed, 45 subtests); final fix restores doctor catch-all containment + schema-incompatible SQLite WARN regression. Post: [`CURSOR-2026-07-19-gate1-correction.md`](CURSOR-2026-07-19-gate1-correction.md). Runbook: [`EXECUTION-embedding-model-eval.md`](../plans/EXECUTION-embedding-model-eval.md). **Next:** Ryan squash-merges PR #44 and pins the resulting merge SHA as the immutable Gate 1 harness version. **Gate 2 still blocked** — no real pilot / live capture/eval until Ryan authorizes.

...
- **CodeQL Complex Therapy — CONDITIONAL PASS; corrections READY FOR KIRO/Ryan REVIEW (2026-08-16, `plan/2026-08-16-codeql-complex-therapy`):** Who/What: Codex corrected the architecture, execution, VERIFY, and STATUS package to make CodeQL a required merge gate on `main` (successor to Pinwheel). When: fresh planning evidence used Protect Main `19156572` and PR #197 head `740424884f8921f1586f6b82648a0a290be40836`; no implementation or disposable PR exists. Why: requiring the GHAS `CodeQL` results check intentionally inherits its current failure semantics, while required-status membership—not strict policy alone—makes red/missing contexts unsatisfied. How: the corrected package binds review to the full pushed correction SHA, requires fresh context/producer revalidation immediately before PATCH, and defines concrete disposable fixtures with a stop-before-inventing-fallback rule. Ryan reviews; separate Execute and disposable-control grants follow. [`ARCHITECTURE-codeql-complex-therapy.md`](../plans/ARCHITECTURE-codeql-complex-therapy.md) · [`EXECUTION-codeql-complex-therapy.md`](../plans/EXECUTION-codeql-complex-therapy.md) · [`VERIFY-codeql-complex-therapy.md`](../plans/VERIFY-codeql-complex-therapy.md) · [`STATUS-codeql-complex-therapy.md`](../plans/STATUS-codeql-complex-therapy.md). **Resume state:** `CORRECTIONS READY` — Execute not authorized; final full review SHA is supplied in the Codex handoff.

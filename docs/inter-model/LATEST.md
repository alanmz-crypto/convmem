...
- **Claude Watch Parity — NEXT: CURSOR GATE 1 SMOKE CONTAINMENT CORRECTIVE:** Start with the [corrective handoff](CODEX-2026-09-18-claude-gate1-smoke-containment-corrective-handoff.md). [PR #310](https://github.com/alanmz-crypto/convmem/pull/310) merged the on-demand adapter at `aadf137`; Cursor's synthetic CLI smoke is pushed at `e4e41ca` with seven tests passing. The worker does not yet prove every configured output path is scratch-confined before indexing, so Cursor adds a fail-closed preflight and adversarial tests, then routes the exact tip for targeted isolation audit. Real-transcript `files_processed=1` / nonzero-units acceptance stays `NOT_RUN`; Ryan separately owns source fingerprint and one-shot run grants. The [Gate 2 plan](CODEX-2026-09-18-claude-watch-parity-gate2-execute.md), live canary, and production watch promotion remain separate Ryan gates. See the [arc brief](../plans/STATUS-claude-watch-parity.md).
- **Trapdoor Hunt / issue #286 — S0–S3 main integration LANDED:** the isolated Codex JSONL substrate landed on `origin/main` via PR #307 (`d657767`). The earlier [`CURSOR-2026-09-17-issue-286-main-integration-handoff.md`](CURSOR-2026-09-17-issue-286-main-integration-handoff.md) is historical. This merge does not authorize S4/S5, production indexing, watcher/config changes, or a #268 OOM-closure claim.

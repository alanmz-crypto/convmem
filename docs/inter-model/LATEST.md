...
- **Claude Watch Parity — NEXT: KIRO GATE 1 CODE REVIEW:** Start with the [exact-tip review handoff](CODEX-2026-09-18-claude-watch-parity-gate1-review-handoff.md). The Gate 1 adapter is pushed at `cbdc88b` but absent from `origin/main`; no PR or written code verdict was found. Kiro previously PASSed only the [Gate 2 plan](CODEX-2026-09-18-claude-watch-parity-gate2-execute.md) at `a811f58` with four Execute conditions. **Sequence:** Kiro reviews Gate 1 code → Ryan decides PR/merge → Ryan separately grants bounded Gate 2 Execute → Cursor implements isolated route. The live canary remains `NOT_RUN`; production routing and watch/source wiring remain Ryan-gated. See the [arc brief](../plans/STATUS-claude-watch-parity.md).

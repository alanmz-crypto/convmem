...
**Updated:** 2026-08-30 (CG-2 D1R12 formal closure record)

- **[Arc CG-2] D1R12 formal obligation SATISFIED / PASS (2026-08-30, Kiro):** Who/What: Independent review closed the retained-reference-v2 formal gap at corrective SHA `7a8fd76350b7076f5d75e3ad53c7392647b2eac0` on `fix/2026-08-30-cg2-d1-reference-v2-formal-d1r12` (parent runtime PASS base `8897d1358f985e38a1070816189460d980824d75`). When: Kiro PASS on frozen delta `8897d13..7a8fd763`, 2026-08-30. Why: prior `GRb*` invariants were trivial stubs; corrective wires substantive reference-v2 semantics and negative controls. How: VERIFY D1R12 row updated to **SATISFIED / PASS**; **fresh empirical TLC rerun at corrective tip deferred** (no Java/TLC in review lane — evidence-strengthening caveat only). **Does not authorize production D1, V8c, or downstream gates; D1R0–D1R11 remain PENDING.** Evidence: [`VERIFY-cg2-production-activation.md`](../plans/VERIFY-cg2-production-activation.md) D1R12 row and closure record.

- **[Arc CG-2 Design A → Production Activation → D1 `G_rb` architecture corrective] REFERENCE-V2 PLAN READY FOR INDEPENDENT REVIEW (2026-08-29):** Who/What: Codex Sol translated Ryan's locked correction into one architecture/execution/runbook/VERIFY package at core planning commit `8b5b53e2a460753711392379535b127cefa244b8` on `plan/2026-08-29-cg2-d1-reference-v2-corrective`. When: after production convert-v1 refused at equivalence and two independent reviews rejected the sidecar corrective. Why: qualification and rollback serving must consume one authoritative physical vector state. How: `G_rb` is now a deterministic reference-v2 target to the exact original D0-covered Chroma rows; the D0 chain/profile remain unchanged; one target-aware reader governs cold proof and serving; recovery coverage is mandatory; failed `2d01dfca…` stays untouched/quarantined/ineligible; property-map-v3 and formal invariants are specified as later implementation work. **Next: independent Kiro review of the exact branch tip, then Ryan plan acceptance; no implementation or production operation is authorized.** Evidence: [`ARCHITECTURE-cg2-production-activation.md`](../plans/ARCHITECTURE-cg2-production-activation.md) · [`EXECUTION-cg2-design-a.md`](../plans/EXECUTION-cg2-design-a.md) · [`RUNBOOK-cg2-production-activation.md`](../plans/RUNBOOK-cg2-production-activation.md) · [`VERIFY-cg2-production-activation.md`](../plans/VERIFY-cg2-production-activation.md).

- **Watch OOM subprocess cap — READY_FOR_PR (2026-08-29, Kiro → Cursor):** Who/What: Cap each watch `index --file` child in its own `systemd-run --user --scope` (MemoryMax=2G, MemorySwapMax=0) so a runaway index OOM kills only the child, not `convmem-watch.service`. When: Ryan verbal auth 2026-08-29 after recurring OOM (2026-08-28 23:50, 4G peak). Why: parent + unbounded index child share cgroup under MemoryMax=4G with no swap. How: branch `fix/2026-08-29-2026-08-29-watch-index-subprocess-memcap` tip `b0d2a8f`; graceful fallback when `systemd-run` absent; serialized spawns unchanged. **Resume state:** `READY_FOR_PR`. Evidence: [`KIRO-2026-08-29-watch-oom-subprocess-cap-handoff.md`](KIRO-2026-08-29-watch-oom-subprocess-cap-handoff.md).

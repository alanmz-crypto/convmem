...
- **CI Behavioral Merge Gate — MERGED, closeout package drafted (2026-08-15, [#187](https://github.com/alanmz-crypto/convmem/pull/187)):** Who/What: hermetic pytest job in GitHub Actions; `Protect Main` ruleset now requires `pytest (3.12)` alongside `pylint (3.12)`. When: #187 squash-merged to `main` as `c2c6429` 2026-08-15. Why: 1,284+ tests existed but never gated merge — a PR could pass CI while hundreds of tests failed. How: `pylint.yml` pytest job with temp config, no live services; ruleset updated same day. **Closeout package on Codex branch:** architecture decision, advisory critical-invariant manifest, VERIFY record, and next-agent handoff are committed. **Still BLOCKED_ON_RYAN:** one disposable failing PR must show `pytest (3.12)` red and ordinary merge blocked; CodeQL is passing but is not currently required by `Protect Main`. [`CODEX-2026-08-15-ci-behavioral-merge-gate-closeout-handoff.md`](CODEX-2026-08-15-ci-behavioral-merge-gate-closeout-handoff.md) · originating [`KIRO-2026-08-15-ci-behavioral-merge-gate-closeout-handoff.md`](KIRO-2026-08-15-ci-behavioral-merge-gate-closeout-handoff.md)

...
- **[Arc Naturalistic ConvMem product-value evaluation] G5 LANDED / G6 Ryan-LOCKED (2026-08-30):** Who/What: G5 squash-merged via [PR #259](https://github.com/alanmz-crypto/convmem/pull/259) at `6843bbeebbaed6a109fe94967fdd03fb3569b583`; post-merge routing at `676d6b5` via [PR #261](https://github.com/alanmz-crypto/convmem/pull/261). When: after Kiro PASS at `23b2495927a9891070c7c294e45bdb641eaab352` and Ryan merges. Why: methodology dry-run is complete; prospective study remains gated. How: hermetic T0–T10 dry-run on `main`; synthetic `0.3` stays descriptive and `blocked_non_estimable`. **Classification: methodology validation, not product evidence. Ryan locked G6 closed until independent ChatGPT review — favorable synthetic results do not open G6.** Resume state: `BLOCKED_ON_RYAN` (ChatGPT review GATE). **Next:** ChatGPT review packet, then Ryan explicit G6 grant if warranted, or park arc. Evidence: [`STATUS-naturalistic-product-value.md`](../plans/STATUS-naturalistic-product-value.md) · [`EXECUTION-naturalistic-product-value.md`](../plans/EXECUTION-naturalistic-product-value.md)
...
- **[Arc Trapdoor Hunt] Repeated watch index-child OOM investigation (2026-08-31):** Who/What: Codex handed Kiro a focused investigation/design brief after repeated transient index-scope OOM kills. When: latest evidence is `run-p390909-i408468.scope` at 16:36:03 CDT, with a prior `run-p354623-i339333.scope` at 16:28:07; both hit the configured 6 GiB child limit while the watcher survived. Why: the existing subprocess isolation mitigates parent blast radius but does not prevent expensive full-file indexing from exhausting the child. How: Kiro should measure the dominant allocation and specify a coverage-preserving mitigation (incremental or bounded work, timeout/backlog controls as justified); no runtime or database mutation is authorized from this handoff. **Resume state:** `NOT_STARTED` (Kiro investigation). Evidence: [`CODEX-2026-08-31-kiro-watch-oom-followup-handoff.md`](CODEX-2026-08-31-kiro-watch-oom-followup-handoff.md).

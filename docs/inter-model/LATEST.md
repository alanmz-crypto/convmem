...
- **Claude Watch Parity — Gate 2 plan READY_FOR_KIRO_REVIEW:** Ryan assigned this new arc on 2026-09-18. Codex's [Gate 2 execution plan](CODEX-2026-09-18-claude-watch-parity-gate2-execute.md) and [arc brief](../plans/STATUS-claude-watch-parity.md) are on `plan/2026-09-18-claude-watch-parity`; Kiro reviews the exact pushed tip next. Gate 1 on-demand adapter is approved but absent from this branch's `origin/main` base. The isolated incremental substrate landed via PR #307 (`d657767`); Claude has no format spec or watch path yet. No Gate 2 Cursor implementation, live canary, `KIRO_ROUTE_FORMATS` change, or `[sources].paths` / `[watch].extra_paths` change is authorized by this plan.

...
| # | Date | Built plan | Skipped original | Status |
|---|------|------------|------------------|--------|
| 1 | 2026-06-24 | `convmem_lauer_roadmap_fed70be7.plan.md` | convmem_improvement_brainstorm_7bc56912.plan.md (same-day brainstorm) | Mostly shipped — see docs/ROADMAP-DRAFT.md (P0–P1b complete; P2 gated) |
| 2 | 2026-06-25 | `GLOBAL-CONVMEM-PROTOCOL-PLANNER.md` | docs/inter-model/*gap-analysis* and critique docs | Shipped — all todos done |
| 3 | 2026-06-25 | `docs/inter-model/PLAN-2026-06-25-surface-coverage.md` | — (post-soak follow-on, not a revision) | Crush/Continue/Kiro done; Cursor soak ongoing |
| 4 | 2026-06-28 | `kiro_echo_permissions_1d403638.plan.md` | — | Shipped — all todos completed |
| 5 | 2026-06-28 | `protocol_gap_fixes_2eb5f259.plan.md` | — | Shipped — all todos completed |
| 6 | 2026-06-29 | `cross-project_synth_agent_aa58015f.plan.md` | — | Decision: defer Phase 2 autonomous agent; Phase 1 digest shipped |
| 7 | 2026-06-29 | `searchable_cli_chats_ffb9fcf6.plan.md` + [PLAN-2026-06-29-searchable-cli-chats-HANDOFF.md](PLAN-2026-06-29-searchable-cli-chats-HANDOFF.md) | searchable_cli_chats_a8f383f4.plan.md (wrong "Kiro IDE" label) | Code shipped; Ryan backfill + Phase 3 verify pending |
...
**Pointer:** [`LATEST.md`](../../SYNTHESIS-STATUS.md) — agents read this first; defers to this section for synthesis.

| Phase | Status |
|-------|--------|
| Phase 0 manual digest | In progress — pilot runs **2–3 done**, run **4** pending ([`CROSS-PROJECT-DIGEST-PILOT.md`](CROSS-PROJECT-DIGEST-PILOT.md)) |
| Phase 1 `cross-project-digest.sh` | **Shipped** (read-only reporter; `--propose` not enabled) |
| Phase 2 autonomous linker | **Deferred** — gates below; **not** the same as ROADMAP P1c ask streaming |
| Phase 3 change feed | Not started |

**Do not conflate tracks** (`dec_prop_20260629_213047_8f73`): background linker Phase 2 = `cross-project-digest.sh --propose`. ROADMAP **P1c** = partial `ask` synthesis on 45s timeout — orthogonal, ship per [`ROADMAP-DRAFT.md`](../../ROADMAP-DRAFT.md) + [`PLAN-2026-06-29-streaming-synthesis.md`](PLAN-2026-06-29-streaming-synthesis.md).
...
2. **Protocol ledger anchors** — **partially done** (see filed table). Multi-prompt checklists must be one approved `dec_prop_*`, not transcript chunks only.
3. **Inter-model prose** — `docs/inter-model/*.md` still not watch-indexed; workaround: `obs_806985bc5697` points here; manual ingest or future watch path still open.
...
- Pilot run **4** (manual or weekly timer) before enabling `--propose` in digest unit
- **Agent habit** on surfaces that still skip convmem (synthesis value gate; see `dec_prop_20260629_213047_8f73`)
- **Inter-model `*.md` watch** — coordination handoffs not auto-searchable
- **P1c** streaming synthesis — next coding item on roadmap (separate from linker Phase 2)

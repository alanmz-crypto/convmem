...
| Naturalistic product-value evaluation | G1–G5 are **LANDED on `main`**, and methodology gate C remains open. Ryan accepted the corrected design represented by Kiro-PASSed `b6a1ccf`, wording-only `fde840b`, and routing `9c58325` as **design-only**. | A later separate Ryan grant is required for bounded G5C implementation, followed by fresh independent review. No G5C code, G6/T0, parameter freeze, natural evidence/corpus access, Agent A/B, scoring, or product interpretation. See [`STATUS-naturalistic-product-value.md`](../plans/STATUS-naturalistic-product-value.md). |
...
is landed but gate C remains open; the corrected design is Ryan-accepted while
every implementation/live step remains separately closed. Recovery/CG-2 remain

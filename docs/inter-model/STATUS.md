...
| Naturalistic product-value evaluation | G1–G4 remain **LANDED on `main`**. G5 synthetic dry-run is a **candidate awaiting Kiro** on `feat/2026-08-30-naturalistic-g5-dry-run`; classified as methodology validation, not product evidence. Arc brief: [`STATUS-naturalistic-product-value.md`](../plans/STATUS-naturalistic-product-value.md). | Independent Kiro G5 review of the exact candidate SHA. No G6, Agent A/B, episode collection, live ConvMem, threshold freeze, or product conclusion. |

...
| Trapdoor Hunt — T3 provenance trust substrate / issue #268 operational follow-up | Provenance T3 remains **CLOSED** and is not reopened. PR #302 removed the proven whole-export RAM accumulator and squash-merged as `a91bb28b`. A follow-up phase profile isolated the next corpus-sized floor in brief refresh: repeated full `collection_metadata_rows()` reads retain unused duplicated provenance envelopes. Plan tip `7bffdfd`; Kiro conditionally PASSed the narrow projected single-pass design with projection pinned (including `rationale`). Ryan authorized Cursor Execute C0–C7 on 2026-09-13. | **Cursor Execute** in a fresh worktree (`~/Projects/convmem-watch-oom-brief-metadata`, branch `impl/2026-09-13-watch-oom-stream-brief-metadata`); resume [`CURSOR-2026-09-13-watch-oom-stream-brief-metadata-execute-handoff.md`](CURSOR-2026-09-13-watch-oom-stream-brief-metadata-execute-handoff.md). Stop at pushed evidence for Copilot audit — no PR yet. No production profiling/access, watcher/config/exclusion operation, source re-inclusion, or Arc Codex P2 progression. After a later reviewed fix, a fresh hermetic end-to-end memory check must precede any watcher or P2 reconsideration. Bootstrap, migration/backfill, CG-2 activation, Shadow/R2b, GC, T4, and T5 remain separately governed and unauthorized. |

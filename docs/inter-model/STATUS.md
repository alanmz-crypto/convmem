...
| Naturalistic product-value evaluation | G1–G5 are **LANDED on `main`**, and methodology gate C remains open. Ryan granted only the strict bounded G5C Cursor allowlist against accepted carrier `c089070`; implementation is `NOT_STARTED`. | Cursor implements on `fix/2026-08-30-naturalistic-g5c-corrective`, pushes evidence, and stops for fresh exact-tip Kiro review. No G6/T0, parameter freeze, natural evidence/corpus access, Agent A/B, live scoring, product interpretation, or merge authority. See the [Cursor handoff](CODEX-2026-08-30-naturalistic-g5c-cursor-handoff.md) and [`STATUS-naturalistic-product-value.md`](../plans/STATUS-naturalistic-product-value.md). |
...
is landed but gate C remains open; bounded G5C implementation is granted but
not started, while every live step remains closed. Recovery/CG-2 remain

...
| CG-2 production activation | Lock candidate `e680ce8`; triple delta PASS (Kiro/Crush/Cursor); Ryan Architecture HITL pending; production remains legacy | Ryan Architecture HITL lock, then Codex execution + VERIFY plans. No implementation, gateway soak, owner activation, or GC yet. See [`STATUS-cg2-production-activation.md`](../plans/STATUS-cg2-production-activation.md). |

...
| Trapdoor Hunt — T3 provenance trust substrate / issue #268 operational follow-up | Provenance T3 remains **CLOSED** and is not reopened. Separately, issue #268's watch child OOM recurred; hermetic evidence identifies global in-memory export compaction as a direct corpus-sized accumulator. The narrow corrective plan received Kiro CONDITIONAL PASS at `90beef5`; its sole bounded-read clarification is now on `plan/2026-09-13-watch-oom-bounded-export-compaction`. | Kiro performs a targeted recheck of the clarification only. No implementation, live compaction, watcher/config operation, source re-inclusion, or P2 progression is authorized. Bootstrap, migration/backfill, CG-2 activation, Shadow/R2b, GC, T4, and T5 remain separately governed and unauthorized. |
...
Kiro-PASSed, but P2 grant progression is paused on issue #268. The narrow
bounded-memory export-compaction plan has one Kiro condition corrected and
awaits targeted recheck; no implementation, live Gate 0/P2, grant digest,
watcher operation, or activation is authorized.

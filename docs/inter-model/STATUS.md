...
| Trapdoor Hunt — T3 provenance trust substrate / issue #268 operational follow-up | Provenance T3 remains **CLOSED** and is not reopened. PR #302 removed the whole-export RAM accumulator (`a91bb28b`); PR #303 merged projected brief metadata streaming as `5c103aa…`. The post-merge diagnostic measured an approximately 2x reduction but isolated the residual envelope-sized reader in `doctor._exposure_window_probe()`. Kiro PASSed the narrow projected-read plan at exact tip `5672ee9`; the Cursor handoff is prepared. | **BLOCKED_ON_RYAN:** Ryan must separately authorize Cursor Execute C0–C7 using [`CURSOR-2026-09-15-watch-oom-bound-exposure-probe-execute-handoff.md`](../inter-model/CURSOR-2026-09-15-watch-oom-bound-exposure-probe-execute-handoff.md). No implementation, production profiling/access, watcher/config/exclusion operation, source re-inclusion, or Arc Codex P2 progression is authorized. After a later reviewed fix, a fresh hermetic end-to-end memory check must precede any watcher or P2 reconsideration. Bootstrap, migration/backfill, CG-2 activation, Shadow/R2b, GC, T4, and T5 remain separately governed and unauthorized. |

...
|---|---|---|
| Claude Watch Parity | Gate 1 is merged. Kiro PASSed the Gate 2 descriptor-bound bubblewrap architecture at `290294d` with required Execute conditions C1–C4. Codex folded those conditions into the normative plan: strict control-root create/reopen validation, `CRASH_EXIT`-only recovery evidence, atomic terminal quarantine markers, and pipe-only worker output. Protected coordinator/isolation/Chroma runtime files remain unchanged. | Kiro performs a narrow exact-tip carry-forward check. Ryan then chooses bounded namespace Execute or `NO_GATE2_ROUTE`. No live source, PR, route, watcher, or activation. See [`STATUS-claude-watch-parity.md`](../plans/STATUS-claude-watch-parity.md). |

...
| CG-2 production activation | CG-1 merged; Kiro PASS, Cursor PASS WITH RISKS, Crush PASS WITH RISKS at `1222b1e`; N1–N3 resolved on `plan/2026-08-14-cg2-production-activation`; production remains legacy | Codex completes the formal model, then Kiro/Crush/Cursor delta-confirm one exact architecture/model revision before Ryan Architecture HITL. No execution planning, implementation, gateway soak, or owner activation yet. See [`STATUS-cg2-production-activation.md`](../plans/STATUS-cg2-production-activation.md). |

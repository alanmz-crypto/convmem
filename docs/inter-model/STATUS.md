...
| Trapdoor Hunt — T3 provenance trust substrate / issue #268 operational follow-up | Provenance T3 remains **CLOSED** and is not reopened. PR #302 removed the proven whole-export RAM accumulator and squash-merged as `a91bb28b`. A follow-up phase profile isolated the next corpus-sized floor in brief refresh: repeated full `collection_metadata_rows()` reads retain unused duplicated provenance envelopes. Kiro conditionally PASSed the narrow projected single-pass plan; its projection-precision conditions and the one subsequently identified rendered scalar (`rationale`) are now folded into the plan. | Kiro performs a final targeted confirmation of `EXECUTION-watch-oom-stream-brief-metadata.md`; Cursor Execute remains unauthorized until unconditional PASS and Ryan authorization. No production profiling/access, watcher/config/exclusion operation, source re-inclusion, or Arc Codex P2 progression is authorized. After a later reviewed fix, a fresh hermetic end-to-end memory check must precede any watcher or P2 reconsideration. Bootstrap, migration/backfill, CG-2 activation, Shadow/R2b, GC, T4, and T5 remain separately governed and unauthorized. |
...
Kiro-PASSed, but P2 grant progression remains paused on issue #268. The
bounded export compactor is merged; the next narrow plan addresses repeated
full metadata reads during brief refresh. No implementation, production
access, live Gate 0/P2, grant digest, watcher operation, or activation is
authorized.

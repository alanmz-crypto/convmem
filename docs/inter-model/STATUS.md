...
| Naturalistic product-value evaluation | G1–G5 are **LANDED on `main`**, but Claude found a G5 prospective-frame composition defect and ChatGPT revised the methodology gate to **C — corrective required**. Prior Kiro PASS remains historical exact-tip evidence, not current methodology clearance. | Ryan decides six corrective methodology choices, then may grant Codex a bounded architecture/execution amendment. No corrective code or G6/T0 work. See [`STATUS-naturalistic-product-value.md`](../plans/STATUS-naturalistic-product-value.md) and the [corrective handoff](CODEX-2026-08-30-naturalistic-g5-methodology-corrective-handoff.md). |

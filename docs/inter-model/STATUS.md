...
**Snapshot:** 2026-09-20. Verify the exact repository tip from `origin/main`
...
|---|---|---|
| Claude Watch Parity | Gate 1 is merged. Gate 2 security review failed at `10322a6`; local findings #1 and #3–#6 have an unreviewed corrective at `e007a22`. Finding #2 has a corrected docs-only descriptor-bound bubblewrap architecture with an unbound snapshot vault, fixed mounted identity, post-run digest revalidation, exclusive lifecycle locking, durable quarantine, literal mountinfo-string exclusion, watch-root disjointness, application-specific host identity, and strict test-skip semantics. Protected coordinator/isolation/Chroma runtime files stay unchanged; the expected Claude format-registry addition remains visible. | Kiro reviews `plan/2026-09-20-claude-watch-parity-boundary`. Ryan then chooses bounded namespace Execute or `NO_GATE2_ROUTE`. No live source, PR, route, watcher, or activation. See [`STATUS-claude-watch-parity.md`](../plans/STATUS-claude-watch-parity.md). |
...
| Trapdoor Hunt — issue #286 isolated Codex JSONL transform reuse | Reviewed S0–S3 at `506afc1…` (Copilot/Kiro PASS; plan `6b62f0f…`) remains on `feat/2026-09-17-issue-286-incremental-index`. Integration onto `origin/main` (`18f63db…`) was performed and pushed on `feat/2026-09-17-issue-286-main-integration`; last code commit `5f142e2…` (not the review tip). Prior exact tip `e99856e…`: Kiro PASS (S0–S3 contract), Copilot FAIL (documentation acceptance). | **READY_FOR_RECHECK:** fresh Copilot and Kiro exact-tip reviews on `git rev-parse origin/feat/2026-09-17-issue-286-main-integration` after fetch. See [the bounded integration handoff](../inter-model/CURSOR-2026-09-17-issue-286-main-integration-handoff.md). No PR, merge, live activation, S4/S5, or #268 OOM-closure claim until Ryan decides after review. |
| Trapdoor Hunt — T3 provenance trust substrate / issue #268 operational follow-up | Provenance T3 remains **CLOSED**. PR #302 removed the whole-export RAM accumulator; PR #303 merged projected brief metadata streaming as `5c103aa…`; PR #305 merged the bounded exposure probe as `ef4a7dd…`. The live 12.5 GiB OOM remains unexplained. | **BLOCKED_ON_RYAN:** after the self-contained documentation branch lands, Ryan may authorize Cursor's §9.7 hermetic end-to-end comparison and name its host/lane using [the post-merge measurement handoff](../inter-model/CURSOR-2026-09-17-exposure-probe-postmerge-measurement-handoff.md). No production profiling/access, watcher/config/exclusion operation, source re-inclusion, or Arc Codex P2 progression is authorized. The §9.8 operational/P2 decision remains Ryan-only after measurement, audit, and review. Bootstrap, migration/backfill, CG-2 activation, Shadow/R2b, GC, T4, and T5 remain separately governed. |
...
metadata read correction (plan tip `7bffdfd`). Fresh worktree +
`impl/2026-09-13-watch-oom-stream-brief-metadata`; stop at pushed evidence for
Copilot — no PR yet. Arc Codex P2, watcher operation, and production access
remain unauthorized. Verify `origin/main` when a commit identity matters.

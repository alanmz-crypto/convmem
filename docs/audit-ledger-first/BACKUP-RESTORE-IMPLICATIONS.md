...
## Phase 0 banner (Architecture #115 — required)

**Chroma-first restore is unchanged in Phase 0.** Shadow ledger backup wiring,
restore-order flip (`ledger → validate → rebuild Chroma`), and treating shadow
as a restore source are **not authorized** by landing these audit docs or by
draft Architecture #115 alone. End-state restore redesign waits for cutover
Architecture + HITL.

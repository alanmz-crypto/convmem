...
### Why not compare ledger IDs with Chroma IDs?

Chroma IDs are random UUIDs assigned at projection time. They are not stable across rebuilds. Comparing them is meaningless for equivalence testing.
...
Document text may differ slightly due to keyword generation changes — this is acceptable as long as the content hash matches.

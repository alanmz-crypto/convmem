...
| Dimension | Assessment |
|-----------|------------|
| Implementation complexity | Low — additive only; no production path changes |
| Rollback | Trivial — delete shadow ledger; nothing changed |
| Risk of split authority | None — Chroma remains sole authority during shadow phase |
...
Rationale:
- Zero risk to production during validation.
- Proves reconstructability before committing to ledger-first.
- Identifies gaps in the write-path coverage.
- Low implementation cost (additive shadow writer).
- Natural graduation to cutover once confidence is established.

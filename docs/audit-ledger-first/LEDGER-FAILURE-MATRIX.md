...
- **Post-Chroma / pre-shadow gap:** After Chroma mutation succeeds and before the
  shadow append/`fsync` completes, a crash can leave Chroma ahead of shadow. That
  gap is expected in Phase 0 (Chroma authoritative) and must be inventoried as
  uncovered delta — not treated as silent success.
- **Bounded lock / fsync:** Shadow append uses a bounded flock + flush/`fsync`
  per Architecture; this is **not** a global Tier-1 quiescence lock for backups.
- **Corruption:** Detected ledger/shadow corruption is a **readiness FAIL**, not
  a skippable warning.

...
| Implementation/tests | **Missing**; Execute remains unauthorized |
...
| Core implementation | Not started | Ryan Execute authorization after review |
| Kiro hook installation/live capture | Not started and not authorized | T0/T4 fixture evidence plus exact Ryan external grant |

## 5. Your Role

The current lane is Codex architecture/execution planning. The next model
should review this brief and the two plan documents for invariant coverage,
then route the package to Kiro for design review and Ryan for the two HITL
gates. After Execute authorization, Cursor owns implementation; the reviewer
must verify the exact revision independently. No model should install live
hooks, bulk reindex, mutate Chroma, or change ledger authority from this brief.

## 6. What Remains Before “Live”

1. Kiro reviews the architecture and execution package; resolve only written
   conditions against the same plan revision.
2. Ryan locks Architecture and Execution separately.
3. Cursor proves the Kiro hook contract (T0) before adding usable hook bytes.

...
| `eval_judgebench/identity_registry.py` | Loads `identity-registry-v1.json`; resolves model aliases | Complete (loader only — classify is in `eval_model_identity.py` on branch) |
...
### On branch `fix/2026-08-09-judgebench-arch-lock-chroma-rebuild` (NOT merged)

| File | What it does | State |
|------|-------------|-------|
| `eval_model_identity.py` | `classify_independence(judge, under_test)` → `self/same_family/cross_family/unknown/not_applicable` | Complete, tested |
| `eval_provenance.py` | Comparison signature computation; `needs_rebaseline` detection | Complete, tested |
| `eval_judgebench/runner.py` | Loads cases → J0 → J1 → compare to gold; dry-run with empty corpus | Complete, tested |
| `eval_judge.py` (modified) | Added `--legacy` gate; legacy path isolated from v1 provenance | Complete, tested |
| `tests/test_eval_model_identity.py` | Identity classification tests | Green |
| `tests/test_eval_provenance_signature.py` | Signature stability/change detection tests | Green |
| `tests/test_judgebench_runner.py` | Runner dry-run, gold-immutability tests | Green |
| `tests/test_eval_judge_legacy.py` | Legacy isolation tests | Green |
| `docs/plans/VERIFY-judgebench.md` | CHK-001..008 status updated | CHK-007 PARTIAL (awaits G3 corpus) |
...
| T2–T5 | Identity, provenance, runner, legacy shim | **DONE on branch** | PR not filed |
| PR merge | T2–T5 code on `main` | **BLOCKED** | No PR created yet |
...
**Summary: Code is ~85% done. The gap is not code — it's getting code merged and Ryan populating gold.**

---

## 5. Your Role (read this to know what you're here to do)

**If Ryan sent you here to implement:** The T2–T5 code is already written on the branch. Unless Ryan asks you to revise it, your job is probably to **file the PR** or **fix something specific** Ryan identified.

**If Ryan sent you here to review:** Read the branch diff against `main`. Key questions: Does identity classification enforce `cross_family`-only for canonical runs? Does comparison-signature detect all the fields listed in the architecture? Is the runner truly Chroma-free?
...
**If you don't know why you're here:** Ask Ryan. The most likely next action is filing the T2–T5 PR.

---

## 6. What Remains Before "Live" (sequential)

- [ ] File PR for `fix/2026-08-09-judgebench-arch-lock-chroma-rebuild` (T2–T5)
- [ ] Ryan reviews and merges PR

...
| Field | Value |
|---|---|
| Verify state | **PARTIAL** — G3 and Phase A are merged on `main` (`5f1a3ef`, `0be0a05`); the zero-call live-driver package is branch-only and CHK-007 remains PARTIAL because no calibration ran |
...
| CHK-007 | Conformance scenario: each defined conformance fixture returns the exact expected disposition under the pinned judge | Fixture-by-fixture disposition comparison | **PARTIAL** — synthetic and mocked driver conformance are covered; corpus-backed conformance awaits driver review/PR/merge and the separately authorized calibration experiment |
...
CHK-001/002/003 are met by the merged S1–S9 slice implementation (#144). CHK-004..006 and CHK-008 met by T2–T5 (Cursor). CHK-009/010 and Phase A identity resolution are on `main` at `5f1a3ef`/`0be0a05`. CHK-007 stays PARTIAL until the zero-call driver is reviewed/merged, Ryan authorizes the separate 60-call experiment (3 candidates × 20 calibration cases), and calibration executes; Ryan's G4 selection follows the calibration evidence.

## Phase A pre-network branch evidence

| Check | Method | Status |
|---|---|---|
| Frozen-producer identity resolution | Phase A preflight resolves canonical frozen producers through `identity-registry-v2` | **PASSED on `main` at `0be0a05`** — no identity decision remains outstanding |
...
| No network/model/calibration call | Focused test command uses synthetic objects, mocked HTTP clients, and local package code only | **PASSED** — live-driver and JudgeBench focused suites are green; no real HTTP client or inference endpoint is used |
| Provider-bound calibration boundary | Offline evidence builds and validates control envelopes internally, derives exact provider wire fields, accepts only strict provider-specific response envelopes, rejects arbitrary semantic callbacks, and transports only manifest-selected calibration IDs | **PASSED on `main` at `0be0a05`; driver branch evidence adds exact 20-call mock entry-path coverage, no retries/fallbacks, explicit partial status, and remote cost gating from serialized wire-request bounds** |
| Llama model/runtime pinning | Read-only `/api/tags` model digest and `/api/version` serving-runtime identity are required before `/api/generate`, then re-checked after all 20 attempts | **PASSED in mocked entry-path tests; Sol read-only evidence pins digest `46e0c10c039e019119339687c3c1757cc81b9da49709a3b3924863ba87ca666e`, Ollama `0.32.3`, and quantization `Q4_K_M`; no inference was run** |
| CHK-007 corpus-backed conformance | No calibration calls were run and G3 is merged/locked at `5f1a3ef` | **PARTIAL** — driver review/PR/merge, Ryan's separate 60-call authorization (3 candidates × 20 calibration cases), and calibration execution remain required |

...
| Field | Value |
|---|---|
| Verify state | **PARTIAL** — G3 is merged/locked on `main` (#170) and Phase A is merged on `main` (#171); CHK-007 remains PARTIAL because no calibration ran |
...
| CHK-007 | Conformance scenario: each defined conformance fixture returns the exact expected disposition under the pinned judge | Fixture-by-fixture disposition comparison | **PARTIAL** — synthetic conformance in `tests/test_judgebench_contracts.py`; corpus-backed conformance awaits the separately authorized calibration experiment (Phase A merged #171) |
...
CHK-001/002/003 are met by the merged S1–S9 slice implementation (#144). CHK-004..006 and CHK-008 met by T2–T5 (Cursor). CHK-009/010 are met by the G3 lock merged on `main` at `5f1a3ef`. Identity resolution merged on `main` via Phase A `identity-registry-v2` (#171); it is not an unresolved or awaiting-decision gate. CHK-007 stays PARTIAL until Ryan's separate 60-call authorization (3 candidates × 20 calibration cases) and calibration execution (Phase A itself merged #171); Ryan's G4 selection follows the calibration evidence.

## Phase A evidence (merged #171; pre-network)

| Check | Method | Status |
|---|---|---|
| Frozen-producer identity resolution | Phase A resolves canonical frozen producers through `identity-registry-v2` | **IMPLEMENTED on `main` (#171)** — no identity decision remains outstanding |
...
| Provider-bound calibration boundary | Offline evidence builds and validates DeepSeek and Llama requests internally, accepts only strict provider-specific response envelopes, rejects arbitrary semantic callbacks, and transports only manifest-selected calibration IDs | **PASSED** — code boundary is 20 transport invocations per candidate run; approved experiment is 3 candidates × 20 calibration cases = 60 calls total; no retries/fallbacks and no network/model/calibration calls were run |
| CHK-007 corpus-backed conformance | No calibration calls were run and no provider result exists; G3 is already merged/locked at `5f1a3ef`; Phase A merged #171 | **PARTIAL** — Ryan's separate 60-call authorization (3 candidates × 20 calibration cases) and calibration execution remain required |

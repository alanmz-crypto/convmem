...
snapshot under `sources/claude-capture/runs/<capture-id>.jsonl`, where
...
- after an ordinary exit, the launcher removes that run's snapshot by held
  parent dirfd after the worker fd closes;
...
- cleanup uncertainty or an unexpected entry quarantines the scratch root.
  Replay then requires a new clean root and cannot use outputs from the failed
  evidence run.

Evidence records the capture-id format, source digest, and cleanup outcome in
closed fields, but never records the host snapshot path.
...
Gate 0 canonicalizes every configured watch root and the scratch root before
opening transcript content. Their trees must be disjoint in both directions:
the scratch root cannot be inside a watched root, and no watched root can be
inside the scratch root. Evidence records only a closed
`watch_roots_disjoint=true` result. A scratch root from a failed post-run watcher
...
uses the `primary_convmem_host` enum plus a SHA-256 digest of the host machine
identity; the raw hostname and machine-id are not serialized. The VERIFY
document names the human-readable host role, kernel, and bubblewrap version.

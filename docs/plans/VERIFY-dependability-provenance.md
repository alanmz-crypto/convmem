...
| ID | Required check | Negative control | Result |
|---|---|---|---|
| V4a | Canonicalization is versioned and deterministic across process restarts and supported implementations. | Field order/Unicode/number/escaping/null/list variants and serialization-library changes are checked against golden vectors. | PENDING |

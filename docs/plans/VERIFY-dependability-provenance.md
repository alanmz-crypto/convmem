...
```text
Status:       T3 CLOSED — RYAN_T3_CLOSE PASS; DEFERRED LATER-GATE ROWS REMAIN PENDING
...
Ryan recorded `RYAN_T3_CLOSE — PASS` against final tested implementation
`6dc50d9ec56e016a32c7eddf3d66636b41923ed8`, merged at
`6b9f6d7544710e81f67ae9d6a15e5a8982a7ce6c`. T3-satisfied and governance-
satisfied rows below are now recorded as `PASS`. Rows whose requirements belong
to a separately authorized later gate remain `PENDING` and are explicitly
listed as deferred; `PENDING` is retained because this VERIFY schema has no
formal `DEFERRED` value. T3 closure does not authorize any later gate.
...
**T3 result:** CLOSED. The final evidence and governance closeout are recorded
...
| ID | Check | Result |
|---|---|---|
| V0a | Kiro records `git rev-parse HEAD` and PASS/FAIL with blocking findings. | PASS |
| V0b | Copilot targeted audit reviews the same SHA. | PASS |
| V0c | Ryan records architecture lock and separate Execute grant. | PASS |
| V0d | Implementation VERIFY records exact implementation SHA and baseline. | PASS |
| V0e | Every existing-code path named by the plan resolves from repository root; planned new paths are explicitly labeled planned. | PASS |
| V0f | T1 Trust Baseline and T2 existing-proof inventory/gap analysis are completed and accepted before any T3/provenance implementation grant; Stage 1A/1B is the T3 child slice, and T4/T5 remain parent-sequence work after T3 with no runtime or cloud-policy action implied. | PASS |
| V0g | P1, P2, and P3 each have a distinct Ryan grant, implementation branch/worktree, PR, and gate; no single grant covers multiple slices. | PASS |
| V0h | P0 is outside Full Fathom Five; the canonical parent hierarchy is FF1/T1 → FF2/T2 → FF3/T3 → FF4/T4 → FF5/T5, and the parent structure is frozen against automatic scope additions. | PASS |

## V0A — Separate execution-slice authorization

| Slice | Required authorization record | Required branch/PR separation | Result |
|---|---|---|---|
| P1 policy/identity | Ryan Execute grant naming P1 scope | `impl/2026-08-17-trapdoor-t3-p1`, PR #203 | PASS |
| P2 bindings/continuity | Ryan Execute grant naming P2 scope | `impl/2026-08-18-trapdoor-t3-p2`, PR #204 | PASS |
| P3 assertion/dedupe/retrieval | Ryan T3 P3 Execute grant from `6be6b353740b58b9652dccc1335906fdacd4e568` | `impl/2026-08-18-trapdoor-t3-p3`, PR #205; implementation head `8aa687724cdedf22b4b602f09cbc5e053d22d046`, squash-merged at `ebe0dfc9a17a4288892dce6f10cd6744f6d27315` | PASS |

P1, P2, and P3 have distinct grants, branches, worktrees, and PRs. Their
implementation and final T3 evidence are complete under the closed T3 record.

## V1 — Root authority and policy ownership

| ID | Required check | Negative control | Result |
|---|---|---|---|
| V1a | One policy function owns root and derivation integrity. | Search proves no adapter/caller duplicate calculator. | PASS |
| V1b | Initial production verified-channel inventory is empty and therefore real production roots/descendants are explicitly untrusted-only. | Transcript role/path/process/source type cannot verify origin; any non-synthetic production root remains `untrusted`. | PASS |
| V1c | Empty inputs cannot mint `agent` or `trusted`. | Construct every empty/missing-input form. | PASS |
| V1d | Caller claims cannot self-upgrade. | Claim `user`, `trusted_tool`, `verified`, and `trusted` in text/metadata. | PASS |
| V1e | Legacy/malformed/unknown-policy records, schema/binding identifiers resolving to different semantic specification bytes, or policy/recipe identifiers resolving to different semantic bytes, become untrusted. | Remove each required envelope field, use a future policy/schema version, replace schema/binding/policy/recipe bytes while retaining the identifier, or alter `schema_semantics_sha256`; verification fails closed. | PASS |
| V1f | Producer fields accept only the closed enums and grant no direct authority. | Try unknown classes/assurances and caller-supplied `verified`; validation fails or result is unknown/untrusted. | PASS |
| V1g | Verified producer identity is not factual truth or a preservation contract. | Use verified-producer fixture with untrusted input and lossy transform; result remains untrusted. | PASS |
...
|---|---|---|
| V2a | Exhaustive/property tests cover every input lattice value and transformer class. | PASS |
| V2b | Tested lossless packaging can preserve, but never exceed, least input integrity. | PASS |
| V2c | Every LLM summarize/distill/rewrite output is capped at `agent`. | PASS |
| V2d | Any untrusted contributor makes output untrusted. | PASS |
| V2e | Partial/unknown ancestry makes output untrusted. | PASS |
| V2f | Repeated transformations are monotone and never rise. | PASS |
| V2g | Deterministic but lossy operations do not preserve trust without an explicit tested contract. | PASS |
| V2h | Recursive recomputation verifies every parent/ancestor, schema semantic specification, policy, recipe, and required binding before using integrity, with the entire operation pinned to one immutable authority generation and policy/recipe-history snapshot. | PASS |
| V2i | Missing ancestor, cycle, parent/commitment mismatch, unavailable historical policy/recipe/schema semantics, loss or mutation of the bound snapshot, bound-state integrity failure, or loss of an actively pinned state yields `untrusted` or a restarted operation; publication of a newer generation alone does not invalidate the pinned traversal, and no favorable mixed-snapshot or substituted-state result is possible. Negative control: reclaim or compact the bound generation/snapshot during an active pin; the operation must detect loss and fail closed/restart, or the pin-retention mechanism must prevent reclamation. | PASS |

## V3 — Exact transformation-boundary binding

| ID | Required check | Negative control | Result |
|---|---|---|---|
| V3a | Binding includes stable source locator and full raw-record hash. | Same content at different records remains distinguishable. | PASS |
| V3b | Binding includes each exact rendered/truncated consumed view. | Change one consumed byte; commitment changes. | PASS |
| V3c | Binding includes message order, selection, chunk, and truncation parameters. | Reorder or change budget; commitment changes. | PASS |
| V3d | Binding includes complete provider payload and fixed recipe/config hash. | Change prompt/tool/retrieval input; payload hash changes. | PASS |
| V3e | Requested/resolved provider/model, fallback, temperature, transformer version, and immutable transformer artifact/build identity are bound; the artifact identity is mandatory for a transformer eligible for the `trusted` cap. | Trigger fallback or substitute an artifact under the same mutable version label; identity/commitment reflects the resolved path and trusted-cap validation fails without the immutable artifact identity. | PASS |
| V3f | Secrets are excluded without excluding semantics-bearing request bytes. | Fixture scans serialized envelope for test credential and required payload bytes. | PASS |
| V3g | `complete` means supported-boundary inputs, not universal model causality. | Documentation and schema avoid impossible claim. | PASS |
...
| ID | Required check | Negative control | Result |
|---|---|---|---|
| V4a | A strict validated typed-envelope acceptance profile precedes canonicalization; the envelope/binding/canonicalization contract has immutable semantic identity; canonicalization then uses one normative, versioned ConvMem profile/serializer and is deterministic across process restarts. | Duplicate keys, invalid/lone Unicode surrogates, NaN/Infinity, out-of-schema numbers, undefined representations, or same-version/different-semantic schema bytes are rejected before canonicalization; serializer golden vectors still fail after serializer-library drift, and no cross-implementation portability is assumed. | PASS |
| V4b | Unit → Chroma → export → reconstruction preserves envelope/commitment exactly. | Remove/alter flat or canonical field; degrade/fail as specified. | PASS |
| V4c | Flat metadata cannot override canonical envelope. | Give scalar cache a more favorable tier than recomputation. | PASS |
| V4d | Effective-integrity cache mismatch degrades to untrusted. | Tamper only the cache. | PASS |
| V4e | Old consumers cannot silently drop provenance and return trusted. | Run old/missing-field fixture. | PASS |
| V4f | Representation, propagation, export, and reconstruction continuity are mandatory. | Remove a required field; fail/degrade to untrusted rather than advisory PASS. | PASS |
| V4g | Future complete-data-v3 preflight names `provenance/` in `STATE_SPECS` and `writer_census_for_root()` as a required Tier-1 durable path, and `docs/RECOVER.md` classifies it accordingly. | Valid registry present in a complete-data-v3 scratch restore returns non-BLOCKED for the provenance path. Historical complete-data-v2 snapshots remain valid under the legacy v2 contract without `provenance/`. | PENDING |
...
| V4m | Capture/sealing consistency: a provenance generation is sealed only from one consistent logical source state, with every manifest-bound authority component covered by the consistency proof. P1 must lock the complete mutator census and consistency mechanism; each later phase must revalidate the census for writers it introduces or changes; V4m reaches PASS only after the final implemented writer set has universal evidence covering every mutator class and representative overlaps, or proves every authoritative writer enters one immutable staging boundary, before T3 arc closure. | Mutate a manifest-bound authoritative source in each classified mutator class and across representative class overlaps, including writers introduced or changed by later phases; without universal staging or equivalent proof, any bypass or incoherent candidate must retry, reject, or quarantine and may not become complete authority. | PASS |

## V5 — Dedupe and assertion identity

| ID | Required check | Result |
|---|---|---|
| V5a | Identical cross-provenance content remains independently auditable. | PASS |
| V5b | Low-integrity duplicate cannot lower/erase a trusted assertion. | PASS |
| V5c | High-integrity duplicate cannot elevate/erase an untrusted assertion. | PASS |
| V5d | Equivalence creates no aggregate trusted assertion. | PASS |
| V5e | Semantic cross-provenance tombstone requires human adjudication and retains both audit assertions. | PASS |
| V5f | Storage optimization cannot destroy provenance identity. | PASS |
| V5g | Assertion IDs are content-independent UUIDv4 values with 122 random payload bits, monitor-minted, atomically reserved, collision-checked, and immutable. **Negative control:** caller attempts to mint, rewrite, recycle, or force a colliding ID. | PASS |
| V5h | Export → reconstruction → re-import preserves a valid ID/commitment pair as idempotent replay only. **Negative control:** round-trip comparison of ID, canonical envelope, and commitment; replay adds no assertion or corroborator. | PASS |
| V5i | Same content without a valid existing ID/commitment pair becomes a new independent assertion. **Negative control:** ingest identical content twice from different roots. | PASS |
| V5j | Parent IDs and commitments match resolved parents. **Negative control:** alter/remove parent commitment or bind the wrong parent ID. | PASS |
| V5k | Invalid identity replay cannot retain, overwrite, alias, or mutate the supplied existing ID. **Negative control:** supply an existing ID with missing/malformed/mismatching commitment or divergent envelope; identity-preserving import fails. If content is retained, it receives a fresh monitor ID and untrusted provenance. | PASS |
| V5l | A parent edge is immutable identity plus expected commitment, not content equivalence. **Negative control:** replace a parent with same-content assertion under another ID; recursive verification returns untrusted. | PASS |
...
| ID | Required check | Negative control | Result |
|---|---|---|---|
| V7a | Stage 1 retrieval returns provenance per assertion. | Mixed-integrity result set has no aggregate trusted label. | PASS |
| V7b | Existing ranking/source-trust scores do not feed effective integrity. | Hold evidence fixed and vary ranking/path metadata. | PASS |
| V7c | Temporal/supersession status does not feed integrity. | Vary timestamps/superseded state with same provenance. | PASS |
...
|---|---|---|---|
| V8a | Untrusted external chunk → LLM summary | untrusted | PASS |
| V8b | Trusted code/tool echoes untrusted input | untrusted | PASS |
| V8c | Same root summarized by two models | one root lineage; no independent corroboration/elevation | PASS |
| V8d | Derivation omits one parent | untrusted/incomplete | PASS |
| V8e | Untrusted retrieval → conversation → recapture → distill | never rises above untrusted | PASS |
| V8f | Prompt/content says `origin=trusted` | no metadata effect | PASS |
| V8g | Provider omission/value/fallback fault | explicit incomplete/degraded result; no elevation | PASS |
| V8h | Child envelope/commitment remains valid-looking while an ancestor is removed | recursive verification returns untrusted; child is not treated as a new root | PASS |
| V8i | Complete-data-v3 restore contains a valid `provenance/` registry and normally captured JSONL/Chroma recovery components | Updated restore preflight classifies the registry as required Tier-1; valid authority may become projection-pending when a rebuildable projection is absent, but projection-backed serving remains blocked | PENDING |
...
|---|---|---|
| V9a | Focused provenance tests and full pytest pass at exact SHA. | PASS |
| V9b | Formatting/static checks and `git diff --check` pass. | PASS |
| V9c | Changed-file inventory contains only authorized implementation/docs/tests. | PASS |
| V9d | Existing retrieval ranking behavior is unchanged. | PASS |
| V9e | No live corpus/Chroma mutation, Shadow activation, R2b capture, or CG-2 operational action occurred. | PASS |
| V9f | Runnable documentation distinguishes reference commands from commands requiring Ryan grant. | PASS |
| V9g | New checks are justified by a distinct failure window, owner, or oracle. **Negative control:** existing standing checks are reused; no duplicate governance ceremony is introduced. | PASS |
...
| ID | Required evidence | Result |
|---|---|---|
| V10a | Kiro implementation design verdict names exact SHA. | PASS |
| V10b | Copilot safety/isolation verdict names the same SHA. | PASS |
| V10c | Material conflicting PASS/FAIL, if any, follows the team-charter Sol-High gate. | PASS |
| V10d | Residual risks name consequence, owner, and disposition. | PASS |
| V10e | Ryan records `RYAN_T3_CLOSE — PASS`; migration/activation remains a separate grant. | PASS |

## Deferred beyond T3

The VERIFY table intentionally retains `PENDING` for requirements that belong
to later separately authorized gates. This schema has no formal `DEFERRED`
result, so these rows are not technical T3 failures and must not be promoted
to `PASS` by this closeout:

| VERIFY row(s) | Explicit disposition | Owning later gate |
|---|---|---|
| V1h | Deferred; production remains untrusted-only until the bootstrap exists | Verified Ingress Bootstrap |
| V3i | Deferred; no migration execution occurred in T3 | Migration |
| V4g–V4j | Deferred; complete-data preflight and authority recovery are later work | Recovery / complete-data integration |
| V4k–V4l | Deferred; rollback integration and broad interruption campaigns are later work | Recovery / T5 |
| V6a–V6e | Deferred; immutable-generation commitment integration is outside this T3 implementation | CG-1 |
| V7d–V7e | Deferred; request-frozen follower serving is outside T3 | CG-2 |
| V8i–V8j | Deferred; complete-data restore and missing-registry recovery are later work | Recovery / CG-1 |
| V8l | Deferred; full authority-first recovery/projection lifecycle is later work | Recovery / CG-1/CG-2 |
| V9A | Deferred; broad egress, endurance, SLO, and operational fault work is later work | T4/T5 |

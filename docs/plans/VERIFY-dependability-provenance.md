...
| V4m | Capture/sealing consistency: a provenance generation is sealed only from one consistent logical source state, with every manifest-bound authority component covered by the consistency proof. | Mutate a manifest-bound authoritative source during generation sealing. Without an immutable/staged generation or equivalent proof, sealing must retry, reject, or quarantine; the incoherent candidate may not become complete authority. | PENDING |

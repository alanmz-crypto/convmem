...
| V2i | Missing ancestor, cycle, parent/commitment mismatch, unavailable historical policy/recipe/schema semantics, loss or mutation of the bound snapshot, or bound-state integrity failure yields `untrusted` or a restarted operation; publication of a newer generation alone does not invalidate the pinned traversal, and no favorable mixed-snapshot result is possible. | PENDING |

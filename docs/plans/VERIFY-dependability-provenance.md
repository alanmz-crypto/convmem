...
| V1e | Legacy/malformed/unknown-policy records, or policy/recipe identifiers resolving to different semantic bytes, become untrusted. | Remove each required envelope field, use a future policy version, or replace policy/recipe bytes while retaining the identifier; verification fails closed. | PENDING |
...
| V2h | Recursive recomputation verifies every parent/ancestor, policy, recipe, and required binding before using integrity, with the entire operation pinned to one immutable authority generation and policy/recipe-history snapshot. | PENDING |
| V2i | Missing ancestor, cycle, parent/commitment mismatch, unavailable historical policy/recipe, or a generation/snapshot change during traversal yields `untrusted` or a restarted operation, never a favorable mixed-snapshot result. | PENDING |
...
| ID | Required check | Negative control | Result |
|---|---|---|---|
| V4a | A strict validated typed-envelope acceptance profile precedes canonicalization; canonicalization then uses one normative, versioned ConvMem profile/serializer and is deterministic across process restarts. | Duplicate keys, invalid/lone Unicode surrogates, NaN/Infinity, out-of-schema numbers, or undefined representations are rejected before canonicalization; serializer golden vectors still fail after serializer-library drift, and no cross-implementation portability is assumed. | PENDING |
...
| V4l | Crash interruption at every durable write, rename, manifest, pointer, recovery, projection-rebuild, and activation boundary leaves either the prior complete authority generation with its valid serving fence or a complete replacement whose projections remain explicitly pending and blocked. | Inject interruption at each boundary; no mixed, missing, partially selected, or stale-fallback authority/projection state may become serving-ready. | PENDING |
| V4m | Capture/sealing consistency: a provenance generation is sealed only from one consistent logical source state, with every manifest-bound authority component covered by the consistency proof; eventual evidence covers every manifest-bound mutator class and representative overlapping mutations across classes, or proves every authoritative writer enters one immutable staging boundary. | Mutate a manifest-bound authoritative source in each mutator class and across representative class overlaps; without universal staging or equivalent proof, any bypass or incoherent candidate must retry, reject, or quarantine and may not become complete authority. | PENDING |
...
| V8k | Historical policy/recipe registry is stale or unavailable, or an identifier resolves to semantic bytes different from its committed digest | recursive recomputation returns `untrusted`; no caller-supplied ID elevates and no same-ID semantic replacement is accepted | PENDING |

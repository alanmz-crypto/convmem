...
| **T4** recovery crash closure | V4l, V9e, V9g | Boundary inventory; interruption matrix; prior-complete or complete-pending/blocked closure; mixed/stale fallback negatives; T5 exclusion | Pending later Execute. **Input for the boundary inventory:** `docs/kernel-panic-durability-insight-2026-08-31.md` records durable-boundary findings from a real unclean panic — the JSONL export write is atomic (temp→fsync→rename under flock) while the Chroma `chroma.sqlite3` + HNSW binaries are NOT covered by ConvMem's atomic helpers, and `doctor`'s `index_drift`/`logical_projection` gates already act as the divergence referee. Steady-state ingest crash-safety is validated; recovery-side interruption (this T4/V4l) is the untested boundary. This is prior-art input only — not T4 execution evidence. |

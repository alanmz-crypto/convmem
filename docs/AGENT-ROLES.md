...
| **Cursor** | Implementer on local workstation after Ryan approves an execution plan; global `convmem.mdc` rule drives session start. Model choice (e.g. Grok) is replaceable inside this lane — not a separate planning lane. | Tier A (shell + MCP) | Yes | feat, fix, docs, wip | No |
...
| **Crush** | Runtime agent with shell (+ MCP when re-enabled). **Weights:** default **Qwen3.7-Max**; also run **DeepSeek V4 Pro/Flash** in Crush when Cursor tokens are exhausted or to burn DeepSeek quota — still Crush lane. See [`MODEL-WORKFLOW.md`](MODEL-WORKFLOW.md) billing-cycle routing. | Tier A (shell; Crush MCP currently disabled — use CLI) | Yes | fix, wip | No |
| **DeepSeek** | Runtime synthesis (`ask` / distill API — already `deepseek-v4-flash`). Separately, **DeepSeek V4 weights inside Crush** = Crush lane (not this row). Separately, **DeepSeek V4-Pro official API** may act as a **Ryan-authorized substitute** for the GitHub Copilot audit lane on one tip+base only via `scripts/deepseek_audit_substitute.py` + [`plans/ARCHITECTURE-deepseek-v4pro-audit-substitute.md`](plans/ARCHITECTURE-deepseek-v4pro-audit-substitute.md) — not Crush, not `convmem ask`, not merge/grant/ledger. | Tier B for `ask`; substitute audit is a Ryan-gated official-API job (not a new Lane) | No | — | No |
...
| **OpenAI Codex** | Default author of approved **architecture and execution plans** (Planning OS Planner function — not a new agent product or Delivery role). Separately installed product; **not** the governing audit lane owner. Default actor for the **PR Steward** Delivery role when Ryan assigns that job (assignment does not enlarge Codex capabilities). Keeps its own factual paths: `~/.codex/AGENTS.md` global + repo root; `codex_rollout_jsonl` adapter; `CODEX-DEEPSEEK-VERIFY.md`; `bash -lc` sandbox network retry; generated filename `codex-agents-convmem.example.md`. Historical posts that say "Codex" for the audit lane refer to the pre-2026-07-19 role and are preserved as-is. | Tier A (shell, no MCP) — use CLI `convmem` commands | Yes | fix, feat, docs | No |
...
**DeepSeek vs Crush:** DeepSeek `ask` = Tier B synthesis only — not a bug-hunter. Crush running DeepSeek V4 weights is still **Crush lane** (Tier A shell). Bug discovery owner = **Crush**, not DeepSeek. A Ryan-authorized **V4-Pro official-API substitute audit** is neither Crush nor `ask`; it must use the architecture + runner above. Full audit: [`docs/inter-model/TEAM-CHARTER-2026-07-06.md`](inter-model/TEAM-CHARTER-2026-07-06.md).

**Lane routing + Sol-High gate:** Architecture and execution planning → OpenAI Codex (Kiro review; Ryan approves); large implementation → Cursor; bound-brief GitHub PR lifecycle → **PR Steward** Delivery role under Ryan HITL (default actor Codex when assigned); investigation/audit/safety → GitHub Copilot audit lane (conditional); Ryan may authorize DeepSeek V4-Pro official API as a tip-scoped substitute via the audit-substitute runner. Sol-High is a separate scarce adjudicator — it requires written PASS or FAIL from both the GitHub Copilot audit lane and Kiro on the same artifact and revision. `defer`, DeepSeek R1 model output, and the embedding worked example's Authorization R1 cannot satisfy the gate. Always-loaded in `TEAM_CHARTER` (`config/agent-protocol.md`). Full lifecycle: [`docs/inter-model/TEAM-CHARTER-2026-07-06.md`](inter-model/TEAM-CHARTER-2026-07-06.md).

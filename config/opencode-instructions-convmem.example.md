...
You have **shell** (`convmem` CLI) and **MCP** (convmem tools via `opencode.json` mcp block) on this machine.
...
- `docs/plans/STATUS-openclaw-convmem-integration.md` — ConvMem Switchboard: read-only OpenClaw-to-ConvMem runtime connector (**architecture drafted; Kiro review not started; no execution plan yet**)
...
## opencode — role and constraints

opencode is a **read-only research and review lane** in this repo. The `opencode.json` permissions deny file edits (except `.opencode/scratch/`) and deny `convmem record` and `convmem index`.

- **Do not** write files outside `.opencode/scratch/`.
- **Do not** run `convmem record`, `convmem index`, or `convmem add` — these are denied by the permission config.
- You are in the **research/analysis** lane: read code, search corpus, ask questions, surface findings.
- Handoff: paste findings into chat for Ryan. No markdown logs unless Ryan explicitly requested one.
- `convmem record` **only** when Ryan says **record block**, **closing**, or **end session** (Ryan runs these).
